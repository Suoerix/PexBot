...
edit_summary = '机器人：从英文维基百科同步船舶相关专题模板到讨论页' # 编辑摘要
...
# --- WPBS 模板名称（小写，用于识别） ---
# 英文 WPBS 名称
en_wpbs_names_lower = {'wikiproject banner shell', 'wpbs'}
# 中文 WPBS 名称 (包括可能的重定向)
zh_wpbs_names_lower = {'wikiproject banner shell', 'wpbs', '专题横幅', 'wpbannershell', 'project banner shell', 'scope shell'}
# 默认使用的中文 WPBS 模板名
default_zh_wpbs_name = 'WikiProject banner shell' # 或者 '专题横幅'
...
# --- 解析函数 ---
def extract_en_wikiproject_templates(talk_page: pywikibot.Page) -> set[str]:
    """
    从英文讨论页文本中提取相关的 WikiProject 模板名称。
    会查找页面顶层的模板和嵌套在第一个 WPBS 内的模板。
    排除 `excluded_en_projects_lower` 中的项目。
    返回一个包含模板名称（不带 Template: 前缀）的集合。
    """
    global error_en_talk_fetch, error_other
    relevant_en_template_names = set()
    try:
        ...
        for tpl in templates:
            ...
            if not wpbs_processed and tpl_name_lower in en_wpbs_names_lower:
                ...
                if tpl.has('1', ignore_empty=True):
                    ...
                    # pywikibot.output(f"...检查 WPBS 参数 1 中的 {len(nested_templates)} 个模板。") # 日志有点多
                    for nested_tpl in nested_templates:
                        ...
                        if is_wp and not is_excluded:
                            relevant_en_template_names.add(nested_tpl_name)
            ...
            # 检查顶层模板是否是需要关注的 WikiProject (排除 WPBS 本身)
                 ...
                 if is_wp and not is_excluded:
                     relevant_en_template_names.add(tpl_name)
    ...
    return relevant_en_template_names

def get_existing_zh_banners(talk_page: pywikibot.Page) -> tuple[set[str], mwparserfromhell.nodes.Template | None, str, mwparserfromhell.wikicode.Wikicode | None]:
    """
    解析中文讨论页，获取第一个 WPBS 模板及其包含的专题横幅信息。
    返回:
    - 一个包含第一个 WPBS 内横幅的*规范化*名称的集合。
    - 第一个找到的 WPBS 的 mwparserfromhell 模板对象 (如果存在)。
    - 页面原始文本。
    - 解析后的 mwparserfromhell Wikicode 对象。
    """
    global error_zh_talk_fetch, error_other
    existing_canonical_banners = set()
    ...
    try:
        if not talk_page.exists():
            ...
            return existing_canonical_banners, None, "", None
        ...
        for tpl in wikicode.filter_templates():
            ...
            if tpl_name_lower in zh_wpbs_names_lower:
                ...
                if tpl.has('1', ignore_empty=True):
                    ...
                    nested_wikicode = mwparserfromhell.parse(str(param1_val))
                    nested_templates = nested_wikicode.filter_templates()
                    # pywikibot.output(f"...检查中文 WPBS 参数 1 中的 {len(nested_templates)} 个模板。")
                    for nested_tpl in nested_templates:
                         nested_tpl_raw_name = str(nested_tpl.name).strip().replace('_', ' ')
                         # 获取规范名并加入集合
                         canonical_name = get_canonical_zh_template_name(nested_tpl_raw_name)
                         if canonical_name:
                             existing_canonical_banners.add(canonical_name)
                         # else:
                             # pywikibot.warning(f"...WPBS 内模板 '{nested_tpl_raw_name}' 无法获取规范名。") # 日志过多
    ...
    if zh_wpbs_template_obj:
         pywikibot.output(f"...已存在于第一个 WPBS 内的规范化专题模板名 ({len(existing_canonical_banners)}): {', '.join(sorted(list(existing_canonical_banners)))}")
    else:
         pywikibot.output("...未在页面上找到 WPBS 模板。")

    # 返回解析结果，包括 wikicode 对象供后续修改
    return existing_canonical_banners, zh_wpbs_template_obj, original_text, wikicode
...
def process_page(en_title: str):
    ...
    global skipped_no_relevant_en_banners, skipped_no_mapping, skipped_no_new_banners
    ...
    relevant_en_template_names = extract_en_wikiproject_templates(en_talk_page)
    if not relevant_en_template_names:
        pywikibot.output(f"未在英文讨论页 '{en_talk_page.title()}' 找到符合条件的专题模板，跳过。")
        skipped_no_relevant_en_banners += 1
        return
    pywikibot.output(f"从英文讨论页找到 {len(relevant_en_template_names)} 个相关模板: {', '.join(sorted(list(relevant_en_template_names)))}")

    # 3. 映射英文模板到中文模板
    target_zh_template_names = set()
    failed_mappings = set()
    for en_name in relevant_en_template_names:
        zh_name = get_zh_template_name_from_en(en_name)
        if zh_name: target_zh_template_names.add(zh_name)
        else: failed_mappings.add(en_name) # ; pywikibot.output(f"...未能映射英文模板 '{en_name}' 到中文模板。") # 日志过多
    if not target_zh_template_names:
        pywikibot.output(f"未能将任何英文模板成功映射到中文模板，跳过页面 '{zh_page.title()}'。")
        skipped_no_mapping += 1
        return
    pywikibot.output(f"成功映射得到 {len(target_zh_template_names)} 个目标中文模板: {', '.join(sorted(list(target_zh_template_names)))}")
    if failed_mappings: pywikibot.output(f"(注意: {len(failed_mappings)} 个英文模板未能映射: {', '.join(sorted(list(failed_mappings)))})")


    # 4. 获取中文讨论页及现有横幅 (现在获取 wikicode 对象)
    ...
    # 调用修改后的函数，获取 wikicode 对象
    existing_zh_canonical_banners, zh_wpbs_template_obj, original_zh_talk_text, wikicode = get_existing_zh_banners(zh_talk_page)
    page_exists = wikicode is not None # 如果 wikicode 不是 None，说明页面存在且已解析

    if not page_exists and not target_zh_template_names: # 如果页面不存在且没有模板要加
         pywikibot.output(f"...中文讨论页 '{zh_talk_page.title()}' 不存在且无需添加模板，跳过创建。")
         skipped_creation_no_banners += 1
         return

    # 5. 确定需要添加的新模板
    new_zh_templates_to_add_raw = set()
    new_canonical_names_added = set()
    for zh_name_raw in sorted(list(target_zh_template_names)):
        canonical_zh_name = get_canonical_zh_template_name(zh_name_raw)
        if canonical_zh_name:
            if canonical_zh_name not in existing_zh_canonical_banners and \
               canonical_zh_name not in new_canonical_names_added:
                 new_zh_templates_to_add_raw.add(zh_name_raw)
                 new_canonical_names_added.add(canonical_zh_name)
        else:
            pywikibot.warning(f"...目标中文模板 '{zh_name_raw}' 无法获取规范名，将无法添加。")

    if not new_zh_templates_to_add_raw:
        pywikibot.output("没有新的、有效的专题模板需要添加到中文讨论页。")
        skipped_no_new_banners += 1
        return
    pywikibot.output(f"最终确定要添加 {len(new_zh_templates_to_add_raw)} 个新模板: {', '.join(sorted(list(new_zh_templates_to_add_raw)))}")
    ...
    # 格式化新模板（无参数，无前导空格）
    # 使用排序后的列表保证顺序一致性
    formatted_new_templates_list = [f"{{{{{name}}}}}" for name in sorted(list(new_zh_templates_to_add_raw))]

    if zh_wpbs_template_obj: # zh_wpbs_template_obj 是 wikicode 中的对象引用
        # --- 情况 A: 合并到现有的 WPBS ---
        pywikibot.output("检测到现有 WPBS，将新模板添加到其参数 1...")
        target_wpbs = zh_wpbs_template_obj # 直接使用获取到的对象引用

        try:
            new_templates_str = "\n".join(formatted_new_templates_list) # 无前导空格

            if target_wpbs.has('1'):
                param1 = target_wpbs.get('1')
                current_value_str = str(param1.value).strip() # 获取现有值，去除首尾空白
                # 组合，确保新旧模板间只有一个换行符
                if current_value_str:
                    combined_value = current_value_str + "\n" + new_templates_str
                else: # 参数存在但为空
                    combined_value = new_templates_str
                # 更新参数值，并确保首尾有换行符以符合常见格式
                param1.value = "\n" + combined_value.strip() + "\n" # strip() 再次确保内部无多余首尾空白
                pywikibot.output("...已将新模板追加到现有参数 1。")
            else:
                # 参数 1 不存在，创建它，并确保首尾有换行符
                param1_value = "\n" + new_templates_str + "\n"
                # 尝试添加到 class 参数后面 (如果 class 存在) 或作为第一个参数
                # mwparserfromhell 的 add() 默认行为是在最后添加，但可以尝试用 after
                added = False
                if target_wpbs.has('class'):
                     try:
                         target_wpbs.add('1', param1_value, after='class')
                         added = True
                     except ValueError: # 如果 'class' 参数找不到 (例如嵌套或复杂情况)
                         pass
                if not added:
                     # 作为后备或首选，直接添加到末尾
            ...
            new_zh_talk_text = str(wikicode) # 从修改后的 wikicode 获取文本

        except Exception as e:
             pywikibot.error(f"!!! 修改现有 WPBS 时出错: {e}。为安全起见，跳过此页面编辑。")
             error_other += 1
             import traceback; traceback.print_exc()
             return # 避免保存可能损坏的页面
        ...
        # --- 情况 B: 创建新的 WPBS ---
        pywikibot.output("未检测到现有 WPBS，将创建新的 WPBS 并添加模板...")
        ...
        # 将新 WPBS 添加到 wikicode 的顶部
        wikicode.insert(0, new_wpbs_text + "\n") # 在开头插入，并加两个换行符分隔

        new_zh_talk_text = str(wikicode).strip() # 获取最终文本并去除可能的多余空白
    ...
    if new_zh_talk_text != original_zh_talk_text:
        ...
        if not dry_run:
            try:
                ...
                zh_talk_page.save(summary=edit_summary, botflag=use_bot_flag)
                edits_made += 1
...
def main():
    ...
    try:
        ...
        skipped_total = (skipped_no_zh_page + skipped_no_en_talk + skipped_en_talk_redirect +
                         ...
                         skipped_no_new_banners + skipped_creation_no_banners)
        ...
        if skipped_no_mapping: pywikibot.output(f"- 因未能将任何英文模板映射到中文: {skipped_no_mapping}")
        if skipped_no_new_banners: pywikibot.output(f"- 因无需添加新模板（已存在或映射失败）: {skipped_no_new_banners}")
